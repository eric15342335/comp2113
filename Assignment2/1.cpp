...
char decryption(int key1, int key2, char letter) {
	...
	// since 4 and 26 are not coprime, there is no modular multiplicative
	// inverse for 4, so we cannot decrypt the message. TLDR: The code should
	// NEVER reach the else part If it reaches, it means the key1 and 26 are not
	// coprime Indicating INVALID KEY
	if (gcd(key1, 26) == 1) {
		...
		// brute force method
		return '!'; // comment this line if you want to try out the brute force
					// method
...
int find_mmi(int a, int mod_value) {
	...
	// Note: This function should never return -1 as you should check their
	// gcd() first before using find_mmi();
}
